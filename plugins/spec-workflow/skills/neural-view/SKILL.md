...
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/neural-view.py" status   # RUNNING <url> notes=N brains=N | STOPPED
...
brains.* Options: `--port N` (default 4748), `--dir ROOT` (brains root; default the git
root — brains live at `<root>/.claude/identities/<role>/brain/`).
