...
1. **Reflect on the ITERATION, not the project.** For each notable thing, ask: did the *workflow* (a skill, a script, a protocol, permissions, review, merge, briefing, board mechanics) help or hurt? Categories: `worked-well`, `friction`, `incident`, `recommendation`. Never file feedback about the project's own code/product — that's a normal board issue or a retro brain note about the project, not this feed. When an item later gets minted as a brain note (by `/retrospective`), the routing rule is: `--confidence direct` when the note is minted from this single concrete incident/feedback item, `inferred` (the default — omit the flag) when it's a cross-item generalization.
