...
5. Commit the page on the task branch — from `$ROOT`, not this skill's cwd (`git -C "$ROOT" add docs/ui-options/<task-id>.html && git -C "$ROOT" commit ...`), so the path in the commit matches where step 2 actually wrote the file.
...
$HUB ask <task-id>-r1 "<task-id>: <short question>" "$ROOT/docs/ui-options/<task-id>.html"
```
...
**Immediately arm the Send-watcher — every `$HUB ask`, unconditionally, not only when no other work remains.** A human clicking Send only reaches you if something is actually watching the outbox at that moment; "keep working, don't block" (step 3) means you will usually be mid-task when they click, so the watcher must not be conditional on being otherwise idle. Right after the `$HUB ask` call above, launch this via Bash with `run_in_background: true` (a one-shot wait, per the Monitor tool's own guidance — not a recurring poll, so it is not a `persistent: true` Monitor):
```bash
UI_HUB_STATE_DIR="${UI_HUB_STATE:-$(git rev-parse --show-toplevel)/.claude/ui-hub}"
until [ -f "$UI_HUB_STATE_DIR/outbox/<task-id>-r<N>.json" ]; do sleep 5; done
```
replacing `<task-id>-r<N>` with the exact id just asked. This exits — and fires a task-notification that reaches you even mid-turn — the instant the human's Send POST lands, regardless of what other (non-UI) work you're doing in the meantime. Every re-`ask` in step 4 gets its own freshly-armed watcher for the new round id; never leave a round unwatched.
...
- **No selection and only UI work remains** → do NOT guess: re-ask `--blocking`, post a reminder comment, confirm the round's watcher from step 2 is still armed (re-arm it if this round wasn't asked/re-asked in this same turn — e.g. resuming a session that ended before step 2's watcher was launched), then report the task blocked-on-human and stop the iteration (see `build-next` stop conditions) — the armed wait, not a scheduled poll, is what resumes it.
