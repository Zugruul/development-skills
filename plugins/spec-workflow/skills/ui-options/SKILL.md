...
- **No selection and only UI work remains** → do NOT guess: re-ask `--blocking`, post a reminder comment, **arm a one-shot wake for the answer** before stopping — a passive "check `$HUB answers` at the next iteration" leaves the loop asleep for a full fallback delay (or requires a manual nudge) even though the human may click Send seconds later. Use Bash with `run_in_background` (a single notification when the condition becomes true, per the Monitor tool's own guidance — this is not a recurring watch, so it is not a `persistent: true` Monitor):
  ```bash
  ...
  ```
  replacing `<task-id>-r<N>` with the exact id just re-`ask`ed. This exits (and fires a task-notification) the moment the human's Send POST lands, waking the wrapping `/loop` immediately instead of waiting out its fallback delay. Then report the task blocked-on-human, and stop the iteration (see `build-next` stop conditions) — the armed wait, not another scheduled poll, is what resumes it.
