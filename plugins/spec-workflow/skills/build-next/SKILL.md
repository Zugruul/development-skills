...
6. Report: task, gate result, PR link, merge/approval state, board status, and a **retro-status line** — `retro: done (notes minted/pruned: ...)` if step 7 ran, or `retro: SKIPPED — <reason>` if it did not (step 7 is MANDATORY at PR close; a skip always needs a stated reason here). When auto-merge ran (step 5), also state the **consent model** the session operated under — `consent: standing (preauth ok)` or `consent: per-artifact (asked, PR #N)` — per `auto-review.md` §3. **Telemetry**: after each review round (step 4), `telemetry.py <root> record '{"kind":"review-round","task":"N","round":R,"verdict":"...","ts":"<now, UTC ISO 8601>"}'`; once the task closes (step 5/QA), `telemetry.py <root> record '{"kind":"task-close","task":"N","estimate":<points>,"ts":"<now>"}'` (estimate from `board.sh show N` / the issue).
7. **Retro — MANDATORY at PR close** (merge OR abandon): interview the dev and reviewer agents, mint/prune/graduate brain notes, regenerate the directory, commit as the orchestrator — full protocol, including `.claude/lessons.jsonl` (SW-020) as retro input, in `${CLAUDE_PLUGIN_ROOT}/skills/build-next/references/brains.md`. Skip only when there is no `.claude/identities/` dir to write into, or a genuine blocker — either way, step 6's report MUST say so via `retro: SKIPPED — <reason>`; a silent skip is never acceptable.
8. **Feedback** (if `methodology.feedback` enabled, distinct from and in addition to step 7's retro — feedback emits/triages a per-iteration process signal, retro mints brain notes; neither replaces the other): invoke the `feedback` skill to record this iteration's process signal. Then, as the ORCHESTRATOR — never a dev agent — triage `feedback.py <root> pending`: dedupe each item's `generalized` text via `python3 scripts/similar.py <root> "<generalized text>"` against existing issues, then `feedback.py <root> route <ts> <idx> <action> <ref>` per item — `backlog` (you create the issue from the generalized text + a `from-feedback` marker, via `board.sh`), `brain-note` (folds into step 7's minting — never a second minting path), `graduate`, `upstream` (surface to the human once), or `ignore` (state why). `methodology.feedback.autoTriage` (default false) gates `backlog` routing on explicit human consent, mirroring `autoMerge`.
