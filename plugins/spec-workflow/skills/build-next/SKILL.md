...
6. Report: task, gate result, PR link, merge/approval state, board status, and a **retro-status line** — `retro: done (notes minted/pruned: ...)` if step 7 ran, or `retro: SKIPPED — <reason>` if it did not (step 7 is MANDATORY at PR close; a skip always needs a stated reason here). **Telemetry**: after each review round (step 4), `telemetry.py <root> record '{"kind":"review-round","task":"N","round":R,"verdict":"...","ts":"<now, UTC ISO 8601>"}'`; once the task closes (step 5/QA), `telemetry.py <root> record '{"kind":"task-close","task":"N","estimate":<points>,"ts":"<now>"}'` (estimate from `board.sh show N` / the issue).
