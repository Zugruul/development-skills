...
- Add the plugin's local-state paths to `.gitignore`. Emit them from the canonical manifest (`scripts/local-state.manifest` — the single source of truth, MEM-010) rather than hardcoding the list: `bash "../../scripts/lib/local-state.sh" ignore >> .gitignore` (the `ignore`-policy entries; `track`-policy paths like `.claude/feedbacks/` are deliberately committed — see below).
