...
1. `python3 "../../scripts/ui-hub.py" answers --consume` (and check the issue's `### UI selection`/`### A11y fixes requested` comments — either channel can carry the answer). The armed watcher is what actually catches the answer as it arrives; this poll is the fallback for a resumed session or a missed watcher, not the primary mechanism.
