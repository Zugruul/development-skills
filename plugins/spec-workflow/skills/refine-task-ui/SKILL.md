...
- **A "UI decision — finalized" section** naming the chosen option and citing the mockup source file.
- **Every captured screenshot**, embedded via `![alt](https://raw.githubusercontent.com/<owner>/<repo>/<default-branch>/docs/ui-options/screenshots/<path>.png)` — a plain relative path does NOT render inline on a GitHub issue (issues aren't repo-relative the way READMEs are); the raw-content URL is required.
