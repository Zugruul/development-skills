...
check "template wires an ambient directional pulse layer for synapse links (batched)" 'synPulses = REDUCED ? null : makeBillboardLayer(CORE_TEX, links.length)' "$(cat "$NVHTML")"
check "ambient synapse pulses are gated by the reduced-motion check" 'REDUCED ? null : makeBillboardLayer(CORE_TEX' "$(cat "$NVHTML")"
check "ambient pulse position is interpolated from the link's live endpoints (l.a/l.b), not a cached copy" "= l.a.x+(l.b.x-l.a.x)*p;" "$(cat "$NVHTML")"
...
check "note halo renders as a batched instanced layer (hover stays screen-space via pickNoteAt)" 'noteHalo = makeBillboardLayer(HALO_TEX, nodes.length)' "$(cat "$NVHTML")"
