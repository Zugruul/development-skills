...
## Follow-up: #236 — gap #4 fix (independent two-pass review)

**A real ambiguity surfaced while designing this fix, worth stating plainly**: `implement-task/SKILL.md` §3 describes TWO distinct review concepts under the same word "review," and this session's own actual practice conflated them — every PR reviewed this session used ONE combined reviewer prompt (spec compliance + code quality together), not two separate agent invocations, despite the SKILL.md text literally saying "Review in two passes, each by a review agent." This fix targets the LITERAL documented invariant (two independent passes exist), and in doing so surfaces that the CURRENT lived practice doesn't fully match the documented protocol — that gap in practice is not something this task silently papers over with a lenient check; the enforcement is written to the DOCUMENTED standard, which will require future implement-task invocations to actually run two separate passes to satisfy it. This is a deliberate, disclosed consequence, not an oversight.

- **Schema change** (`scripts/telemetry.py`): the EXISTING `review-round` event kind gains one new OPTIONAL field, `pass` — one of `"spec-compliance"` / `"code-quality"`, identifying WHICH of implement-task's two review passes this round belongs to. Omitted entirely for merge-dialogue rounds (auto-review.md's separate ≤3-round reviewer-approval process, invariant #9's own concern — NOT this invariant) — this keeps the two review CONCEPTS (implement-task's own two-pass gate vs. auto-merge's independent-reviewer dialogue) distinguishable within the SAME event kind rather than inventing a parallel schema, since both are fundamentally "a review round happened" events. Purely additive: `validate_record()`'s `review-round` branch adds an OPTIONAL check (if `pass` is present, it must be one of the two literal values; absent is valid, matching every existing pre-this-PR telemetry record).
- **New script**: `scripts/two-pass-review-preflight.sh [--root <path>] [--task <id>]` (mirrors `gate-preflight.sh`'s CLI shape). Reads `.claude/telemetry.jsonl` (same file `telemetry.py` already writes/reads), filters to `kind == "review-round"` AND `task == <id>` AND `pass` is one of the two literal values, and requires BOTH `"spec-compliance"` and `"code-quality"` to appear at least once among those records for the task. Missing either → FAIL, actionable message naming which pass is missing. Malformed/missing telemetry file → treat as "zero records," i.e. FAIL with a message pointing at both passes missing (matches `red-first-preflight.sh`'s graceful-empty-input handling, not a crash).
- **Enforcement point — NOT "in review"**: unlike gaps #2/#3, the two-pass review does not happen BEFORE "In review" in this workflow's actual sequencing (build-next's own steps: verify+move-to-In-review, THEN review, THEN merge) — gating "In review" here would be structurally wrong, since no review has happened yet at that point by design. Wire this into `_do_move()`'s **`"qa"`** branch instead (a NEW branch alongside the existing `"in review"` one — QA is the next board.sh-mediated transition after review+merge complete, per `build-next/SKILL.md`'s "Advancing beyond In review" section: *In review* → *QA* happens right after the PR merges). This is the first of the 3 fixes so far to gate a DIFFERENT transition than "in review" — confirm the normalization helper handles `"QA"`/`"qa"` consistently the same way it already does for `"in review"`/`"in progress"`.
- **Also fix, in the SAME PR**: `implement-task/SKILL.md`'s telemetry-recording line (step 4, "after each review round in step 3, `telemetry.py ... record '{"kind":"review-round",...}'`") gains guidance to include `"pass": "spec-compliance"` or `"pass": "code-quality"` for THIS skill's own two-pass review rounds specifically (not the separate auto-merge dialogue rounds in `auto-review.md`, which stay as they are — no `pass` field, unrelated invariant). Keep this a small, additive doc clause, not a rewrite of the whole review section.
- **Test coverage**: (a) both passes recorded for a task → PASS; (b) only `spec-compliance` recorded → FAIL, message names `code-quality` as missing; (c) only `code-quality` recorded → FAIL, message names `spec-compliance` as missing; (d) neither recorded (empty/missing telemetry.jsonl, or records exist for a DIFFERENT task) → FAIL, both named missing; (e) merge-dialogue `review-round` records (no `pass` field, e.g. auto-merge's round 1/2/3) present for the task do NOT satisfy either pass — confirm they're correctly ignored, not miscounted as either pass; (f) regression: moving to any status OTHER than "QA" is unaffected by missing pass records (e.g. "In review" itself, "Backlog", "Ready", "Deployed" all still work without this check firing) — the existing `gate-preflight.sh`/`red-first-preflight.sh` checks on "in review" stay completely independent and unaffected by this new "qa" branch.
- **Known limitation** (state explicitly, do not hide): this verifies TWO review-round records with the right `pass` tags exist — it does NOT verify they came from genuinely INDEPENDENT agent invocations (a single reviewer session that manually records both `pass` values without actually running two separate passes would satisfy this check). Closing that gap would require verifying distinct agent/session identifiers per record, which `telemetry.py`'s schema doesn't currently carry — out of scope here, note as a possible future tightening if this proves to matter in practice.

## Follow-up: #237 — gap #5 fix (identity-brain isolation, orchestrator-mediated only)

**The core problem, restated precisely**: `brains.md` states "only the orchestrator process ever reads or writes a brain… subagents never see a brain path" — but this is a convention (briefs simply never mention the path), not a technical boundary. A subagent that discovers `.claude/identities/<role>/brain/` on its own (e.g. `find .claude`, or blind curiosity) has zero obstacle reading note bodies, `links.json` weights, or the activation log directly.

**Why a session-identity check (orchestrator vs. subagent) was considered and rejected**: the obvious-looking fix is "block the read only when the CALLER is a subagent, allow it from the main/orchestrator session." This requires a reliable signal distinguishing the two in the PreToolUse hook payload. That signal was not verified against this actual harness — the only two prior hook scripts in this repo (`guard-board-move.sh`, `gate-preflight.sh`) never needed to make this distinction, so there is no existing, proven pattern here to copy, and asserting an unverified field name into a security-relevant check would violate this repo's own "grounded in the actual current code, not assumed" standard (CDX-031's own audit method, §"Method" above). Rather than spend this task's whole budget on empirical hook-payload verification, the design sidesteps the need for the distinction entirely (next paragraph) — a strictly simpler and more robust mechanism than caller-identity detection would have been even if verified.

**Design decision: universal enforcement, not session-scoped** — block **direct, raw content access** to `.claude/identities/<role>/brain/**` (the `notes/`, `links.json`, `.activation.jsonl`, `consults.json` contents) for **every** caller, orchestrator session included, not just subagents. This is deliberately *stricter* than the literal invariant text ("orchestrator-mediated only" could be read as "the orchestrator may read directly") but is fully consistent with actual sanctioned usage: nowhere does the orchestrator's own protocol (`brains.md`) call for raw `cat`/`Read` of a note or `links.json` — every legitimate orchestrator operation already goes through `brain.sh`'s subcommands (`recall`, `mint`, `directory`, `consult`, `prune`, `graduate`, …). Forcing 100% of brain content access through that one script, for anyone, closes the gap without needing to trust an unverified per-call identity field. `ROLE.md` and `DIRECTORY.md` are explicitly OUT of the blocked path (they live at `<role>/ROLE.md` and `.claude/identities/DIRECTORY.md`, one level above `brain/`) because `brains.md` requires the orchestrator to read `ROLE.md` **verbatim** and a slice of `DIRECTORY.md` directly into every brief — those two files were always meant to be read as plain text, never gated.

- **New script**: `scripts/guard-brain-access.sh` (mirrors `guard-board-move.sh`'s hook shape: reads hook JSON on stdin, exit 2 + actionable stderr = block, exit 0 = allow). Reads `tool_name` from the hook payload (defaulting to `"Bash"` when absent, so the existing `hookjson()`/`hookjsonpy()` test helpers — which never set `tool_name` — keep working unmodified for `guard-board-move.sh`'s own tests, and so a caller that omits the field fails toward the same coverage this repo already has, not silently toward zero coverage):
  - `tool_name == "Read"`: block when `tool_input.file_path` matches `<anything>/.claude/identities/[^/]+/brain/.*`.
  - `tool_name == "Bash"` (or absent): parse `tool_input.command` the same way `guard-board-move.sh` already does (shlex tokenize, recurse into `bash -c`/`sh -c` wrappers) looking for a content-dumping command (`cat`, `head`, `tail`, `less`, `more`, `sed`, `awk`, `grep`, or a `python3`/`node` one-liner containing `open(`/`readFile`) whose argv includes a path matching the same brain-subtree pattern. A bare `find`/`ls` that only lists paths (no content-dump command) is deliberately NOT blocked — discovering that the path exists is not the harm this fix closes; reading its content is.
  - Any other `tool_name` (`Grep`, `Glob`, `Write`, `Edit`, …): allowed, unconditionally — see "Explicitly out of scope" below.
- **Enforcement point**: `hooks/hooks.json` gains a second entry in `PreToolUse` for matcher `"Read"` (new) running `guard-brain-access.sh`, and the existing `"Bash"` matcher's `hooks` array gains a second command entry (alongside `guard-board-move.sh`, which keeps running unmodified — both hooks fire independently per Claude Code's documented "each PreToolUse hook can independently block" behavior, and neither script's logic depends on the other having run). HOOK-ONLY (Claude-specific), same category as `session-start.sh`'s checkpoint advisory — there is no host-neutral "board.sh"-style single choke point for arbitrary file reads the way there is for board mutations, so, unlike gaps #2/#3/#4, this fix cannot be made SCRIPT-ENFORCED; it is defense-in-depth under Claude Code only, and the gap remains fully open under Codex (no hook-interception equivalent exists there) — state this plainly rather than implying parity.
- **Explicitly out of scope / stated limitations** (do not hide, mirror the disclosure style of #234/#235/#236):
  1. **`Grep`/`Glob` tool calls, and a Bash `grep -r`/`find … -exec cat`/`xargs cat` sweep rooted above `.claude/identities/`, are NOT blocked even though they could incidentally traverse into a brain directory and surface content in matched output.** Blocking every recursive/repo-wide search that could theoretically touch the path would require redacting matched output post-hoc (a materially larger mechanism) or refusing all repo-wide recursive reads outright, which would break unrelated, legitimate work far more often than it prevents brain leakage. This closes the concrete scenario the issue names (`find .claude` to discover the path, then a **targeted** read of the discovered file) without the disproportionate cost of the broader one.
  2. **No caller-identity distinction** — the block is universal (see Design decision above); this is a scope choice, not an oversight, and does not weaken enforcement (it is strictly more restrictive than a subagent-only block would have been).
  3. A sufficiently adversarial one-off (e.g. an obscure interpreter or encoding this repo's content-dump heuristic doesn't recognize) could still slip through — same class of limitation `guard-board-move.sh` itself accepts for unparseable-but-fails-closed input; this fix uses the same fail-open-on-unmatched, not fail-closed-on-uncertain, trade-off as its established sibling, and that asymmetry is itself worth naming: an unparseable **board-move** command fails closed (blocks) because false positives there just cost a retry, whereas an unrecognized **read** command here fails open (allows) because misclassifying an ordinary Read call as a brain-content dump would break unrelated, unrelated-to-this-fix work far more often than it would catch a real bypass attempt.
- **Test coverage** (`tests/section-guard-brain-access.sh`, new — mirrors `section-gate-core.sh`'s hook-testing technique):
  1. `Read` tool call with `file_path` under `.claude/identities/dev/brain/notes/x.md` → blocked, actionable message naming `brain.sh`.
  2. `Read` tool call with `file_path` for `.claude/identities/dev/ROLE.md` → allowed (regression: the always-legitimate verbatim read).
  3. `Read` tool call with `file_path` for `.claude/identities/DIRECTORY.md` → allowed.
  4. Bash `cat .claude/identities/reviewer/brain/links.json` → blocked.
  5. Bash `bash brain.sh recall dev --paths "x/**"` → allowed (the sanctioned interface itself, never flagged).
  6. Bash `find .claude -iname '*brain*'` → allowed (discovery only, no content dump — see limitation 1 above).
  7. Bash `grep -r "foo" .` (repo-wide, not brain-targeted) → allowed (regression + limitation 1).
  8. Bash `bash -c "cat .claude/identities/dev/brain/notes/x.md"` (wrapped) → blocked, same recursion handling as `guard-board-move.sh`.
  9. Unrelated `Write`/`Edit`/`Grep` `tool_name` values → allowed unconditionally, regardless of path (limitation 1/2 made concrete in a test, not just prose).
  10. Hook payload with no `tool_name` field at all (old-style `hookjson()` output) → treated as `Bash`, existing `guard-board-move.sh` behavior/tests stay green and unaffected by this new script's addition to the same matcher.
