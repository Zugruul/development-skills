# Backlog — spec PR (SPEC-peer-review-draft.md)

> **DRAFT — generated by a Fable 5 agent from compute-registry-plan-v3.md, not yet reviewed
> or approved by the human, not registered in `.claude/project.yaml`.**

Task ids: `PR-<number>`. Ranges: E0 = 001–009, E1 = 010–019. Points ≈ complexity (1–10
...
(no cross-epic guards needed). Within E1, PR-011/PR-012 are blockedBy PR-010.

## E0 — `/peer-review` skill (§6)

### PR-001 · Diff-source resolution + preflight — P0 · 3 pts · §6.1 §6.3 §6.4 §6.7
...
### PR-002 · Codex invocation + structured findings with raw fallback — P0 · 4 pts · §6.2 §6.5 §6.6 §6.8
...
### PR-003 · /peer-review SKILL.md + docs + disclosure — P1 · 3 pts · §6.9 §6.10 §12(OQ-1)
The user-facing skill: terse SKILL.md (house style) wiring PR-001/PR-002, argument parsing,
output rendering; placement per OQ-1's answer (default `plugins/peer-review`).
...
OQ-1 pick recorded on the issue before implementation starts (default applies if
unanswered).
**DoD:** suite green; skill listed in plugin README; `claude plugin validate` passes.

*(PR-004–009 headroom for discovered E0 work.)*

## E1 — Remote LLM dispatch (§7 §8)

### PR-010 · Registry file schema + loader/validation — P0 · 3 pts · §7.1 §7.2 §7.3
...
### PR-011 · /compute-registry status skill — P1 · 3 pts · §7.4 §7.5 · blockedBy PR-010
Reads the registry, GETs `<endpoint>/models` with bearer token and per-provider timeout
(default 5 s — OQ-3), reports reachable/unreachable + why; reference doc carries the YAML
template and the exact `llama-server --host 0.0.0.0 --port 8080 --api-key ...` launch
...
**DoD:** suite green; OQ-3 pick recorded (default applies if unanswered).

### PR-012 · Dispatch helper — P1 · 3 pts · §8.1–§8.5 §12(OQ-2) · blockedBy PR-010
`dispatch(provider_id, prompt, timeout_s)` → POST `chat/completions`, bearer auth, returns
text; default timeout 120 s (OQ-2); documented manual invocation against the real notebook
(advisory, not merge-gating — spec §11, OQ-4 default keeps `/peer-review` wiring out of v1).
...
**DoD:** suite green; OQ-2 pick recorded (default applies if unanswered).

*(PR-013–019 headroom for discovered E1 work.)*
